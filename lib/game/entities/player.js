  ...
  'impact.entity',
	'game.entities.chatbubble'
	 ...
	 EntityPlayer = ig.Entity.extend({
        ...
        update: function() {
          ...
          if (this.previousFalling && this.standing) {
            if (this.fallSpeed > 145) {
              this.kill('Falling', 'Velocity: ' + this.fallSpeed);
              return;
            }
          }
        ...
        kill: function( reason, detail) {
          this.parent();
          ig.global._gaq.push(['_trackEvent', 'PlayerDeath', reason, detail]);
        ...
        receiveDamage: function(amount, from){
          ...
          this.health -= amount;
          if( this.health <= 0 ) {
            var killer = from.name;
            if(from instanceof EntityGhost) {
              killer = from.ghostType + ' ghost (' + from.name + ')';
            }
            this.kill( 'Health', 'Final blow: ' + killer);
          }
        } 
    });
