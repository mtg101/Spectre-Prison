...
ig.module(
  ...
  'impact.entity'
	 ...
	 EntityGhost = ig.Entity.extend({
        ...
        setFlipFromMovement: function() {
          if (this.vel.x) { //face right direction, but only change if we're moving
            if(!this.bistableFlip) {  //simple version
              this.currentAnim.flip.x = this.vel.x > 0 ? false : true;
              return;
            }
            //only done for bistable types...
            var newFlip = this.vel.x > 0 ? false : true;
            if(newFlip != this.currentAnim.flip.x) {
              //don't change on first frame to avoid bi-stable flipping
              if (this.flipCount < 2) {
                this.flipCount++;
              } else {
                this.currentAnim.flip.x = newFlip;
                this.flipCount = 0;
              }
            } else {
              this.flipCount = 0;
