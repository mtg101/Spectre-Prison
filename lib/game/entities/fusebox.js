    ...
    EntityFusebox = ig.Entity.extend({
        ...
        setWorking: function() {
          ...
          for (var i=0; i<this.lightbulbs.length; i++) {
              this.lightbulbs[i].setAnim();
          }
        },
        registerLightbulb: function( lightbulb ) {
          if(lightbulb instanceof EntityLightbulb) {
            var len = this.lightbulbs.push(lightbulb);
          } else {
            console.log('registerLightbulb: this isn\'t a lightbulb');
          }
        }
    });  
});
